...
const AnagramPage = () => {
        ...
        const fetchQuestions = async () => {
            ...
            try {
                const response = await fetch(
                    `${base_url}/api/questions?query=&page=${page}&limit=2&type=ANAGRAM`
                );
    ...
    return (
        <div>
            <Search />
            <div>
                <h1 className="text-center font-bold text-2xl m-4">Anagram Questions</h1>
                        ...
                        {questions.length > 0 ? (
                            questions.map((question, index) => (
                                <div key={question._id}>
                                    ...
                                    >
                                        {`${(page - 1) * 2 + index + 1}. ${question.title}`}
                                    </h3>
                                </div>
                            ))
                        ) : (
